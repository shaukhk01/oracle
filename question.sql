...
--Q:SuperMarket Transaction Data In Each Day Example You Have To Understand Slowly?

SELECT  TRANS_ID,TO_CHAR(TRANS_DATE,'DD-MM-YYYY HH24:MI:SS' ) TRANS_DATE ,
PRODUCT_CODE ,QTY* PRICE AMOUNT,
SUM(QTY* PRICE) OVER () TODAY_TOTAL--[[HERE WE USED OVER() BECAUSE OF NOT A SINGLE GROUP FUNCTION]]
FROM SUPERMARKET_TRANS             --[HERE SUM()USED FOR TOTAL OF ROW QTY*PRICE)
WHERE TRUNC(TRANS_DATE)=TRUNC(SYSDATE)



--Q:Each Row RunningTotal:-Best Practice To used This method not above[over()]
    Ex:1 :1
       2 :3
       3 :6
...
FROM SUPERMARKET_TRANS--------------------[here order by disp each row running total because of trans_id]
WHERE TRUNC(TRANS_DATE)=TRUNC(SYSDATE)


--FIRST_VALUE|LAST_VALUE (EXPR) [ { RESPECT | IGNORE } NULLS ]
--Q:DISPLAY FIRST VALUE OF SALARY USING FIRST_VALUE()

*SELECT EMPNO,SAL,FIRST_VALUE(SAL)OVER()FROM EMP ORDER BY SAL;
...
Q:WRITE A QUERY TO DISPLAY FIRST VALUE FROM UNDERSTAND_RESPECT_IGNORE
  +---------------------------------------------------------+
  | SELECT VAL,FIRST_VALUE(VAL) OVER ( ORDER BY VAL DESC )  |
  | FROM UNDERSTAND_RESPECT_IGNORE                          |
  +---------------------------------------------------------+
Q:WRITE A QUERY TO DISPLAY FIRST VALUE AND LAST VALUE FROM EMP TABLE
  +--------------------------------------------------------------+
  | SELECT EMPLOYEE_ID, SALARY , FIRST_VALUE(SALARY) OVER ( ) FV,|
  | LAST_VALUE(SALARY) OVER ( ) LV FROM EMPLOYEES                |
  +--------------------------------------------------------------+
--how oracle execute this

  +--------------------------------------------------------------------+
  | SELECT EMPLOYEE_ID, SALARY , FIRST_VALUE(SALARY) OVER ( ) FV, |    |
  | LAST_VALUE(SALARY) OVER ( ) LV FROM EMPLOYEES ORDER BY SALARY |    |
  +---------------------------------------------------------------+----+

Q:WRITE A QUERY TO DISPLAY FIRSTVALUE AND LASTVALUE FROM DEPTNO 10 AND 20 USING EMP TABLE
   +--------------------------------------------------------------------------+
   |SELECT EMPLOYEE_ID, SALARY ,DEPARTMENT_ID,                                |
   |FIRST_VALUE(SALARY) OVER (PARTITION BY DEPARTMENT_ID ) FV,                |
   |LAST_VALUE(SALARY) OVER ( PARTITION BY DEPARTMENT_ID) LV FROM EMPLOYEES   |
   |WHERE DEPARTMENT_ID IN (20,30)                                            |
   +--------------------------------------------------------------------------+

--THE DBMS_RANDOM PACKAGE PROVIDES A BUILT-IN RANDOM NUMBER & STRING GENERATOR.[see random.sql]

SELECT DBMS_RANDOM.STRING('L',15) FROM DUAL;
here:-'L',15-represent 'L'->lower,15-->character
...
SELECT 'HI :) '||TABLE_NAME FROM USER_TABLES;

--YOU CAN CREATE SCRIPT FOR ALTER TABLE[[see dynamicscript.sql]]

Q:DISPLAY EMPLOYEE AND THEIR MANAGER ALSO

SELECT EMPLOYEE_ID, FIRST_NAME,LAST_NAME,MANAGER_ID
FROM EMPLOYEES  START WITH EMPLOYEE_ID=100
connect by prior employee_id=manager_id --top down 

or
SELECT EMPNO,ENAME,MGR FROM EMP START WITH EMPNO=7698 CONNECT BY PRIOR EMPNO=MGR;

or

SELECT EMPNO,MGR FROM EMP WHERE MGR IN(SELECT EMPNO FROM EMP);
...
SELECT ENAME||'WORK FOR '|| (SELECT ENAME FROM EMP E WHERE E.EMPNO=A.MGR)FROM EMP A START WITH EMPNO=7698 CONNECT BY PRIOR EMPNO=MGR;
...
SELECT LEVEL,EMPLOYEE_ID, FIRST_NAME,LAST_NAME,MANAGER_ID
FROM EMPLOYEES WHERE LEVEL=2 START WITH EMPLOYEE_ID=100
CONNECT BY PRIOR EMPLOYEE_ID=MANAGER_ID 


SELECT LAPD('XYZ',LENGTH(ENAME),'-')FROM EMP;
...
---DML LOG USING PACKAGE ERROR_LOG[[see error_log.sql]]
